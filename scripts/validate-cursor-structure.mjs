...
async function validateComponentFrontmatter(pluginDir, pluginName, skillsField) {
  ...
  const skillDirs = new Set([path.join(pluginDir, "skills")]);
  for (const value of extractPathValues(skillsField)) {
    if (isSafeRelativePath(value) && !value.startsWith("http")) {
      skillDirs.add(path.resolve(pluginDir, value));
    }
  }
  let skillCount = 0;
  ...
  if (skillsField !== undefined && skillCount === 0) {
    addError(`${pluginName}: "skills" is set but no SKILL.md was found under it.`);
  }
...
async function main() {
  ...
  for (const [index, entry] of marketplace.plugins.entries()) {
    ...
    await validateComponentFrontmatter(pluginDir, entry.name, pluginManifest.skills);
