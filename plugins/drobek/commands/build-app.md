...
`restore_version`, `publish`) — do not write the app's files to the local
...
3. Write a real, working first version with ONE `write_files({ app_id, files,
   ...
   `https://esm.sh/<package>@<version>` URLs. Keep all state in the browser;
   there is no backend, and `fetch` to other sites is blocked.
