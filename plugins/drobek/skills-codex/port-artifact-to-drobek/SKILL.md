...
description: Move a Claude artifact (an HTML page, a React component, or a folder with index.html, scripts, images and video) into the user's drobek workspace as a new app, paths unchanged. Use when the user asks to move, port or host a Claude artifact on drobek.
---
...
drobek. The drobek tools must be connected; if they are missing or answer
401, ask the user to run `codex mcp login drobek`, sign in and restart
Codex, then stop.
